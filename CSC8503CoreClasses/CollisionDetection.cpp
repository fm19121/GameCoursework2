...
bool  CollisionDetection::OBBSphereIntersection(const OBBVolume& volumeA, const Transform& worldTransformA,
	...
	Matrix3 orientation = worldTransformA.GetOrientation();
	Matrix3 invOrentation = worldTransformA.GetOrientation().Conjugate();
	Transform OBBTransform = worldTransformA;
	Vector3 delta = worldTransformB.GetPosition() - worldTransformA.GetPosition();
	OBBTransform.SetPosition(Vector3());
	Transform sphereTransform = worldTransformB;
	sphereTransform.SetPosition(invOrentation * delta);
	bool isCollide = AABBSphereIntersection((AABBVolume)volumeA.GetHalfDimensions(), OBBTransform, volumeB, sphereTransform, collisionInfo);
	if (isCollide) {
